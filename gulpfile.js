...
gulp.task('pl-copy:img', function () {
  return gulp.src('**/*.{ico,png,gif,jpg,jpeg,svg,tif,bmp,ico,json,xml}', {
  ...
  return gulp
    ...
    .pipe(
      gulp.dest(function (file) {
        // flatten anything inside the styleguide into a single output dir per
        // http://stackoverflow.com/a/34317320/1790362
...
gulp.task('pl-bundle:img', function () {
  return gulp
    .src('**/*.{ico,png,gif,jpg,jpeg,svg,tif,bmp,ico,json,xml}', {
...
function watch() {
  ...
  var patternWatches = [
    path.resolve(paths().source.patterns, '**/*.{json,md}'),
...
gulp.task('patternlab:watch', watch);
gulp.task('patternlab:clean', gulp.series('pl-clean'));
gulp.task('patternlab:compile', gulp.series('pl-clean:build', 'pl-build'));
gulp.task(
  'patternlab:serve',
  gulp.series('patternlab:connect', 'patternlab:watch')
);
gulp.task(
  'patternlab:bundle',
  gulp.series('patternlab:compile', 'pl-clean:bundle', 'pl-bundle')
);
gulp.task('serve', gulp.series('patternlab:serve'));
gulp.task('clean', gulp.series('patternlab:clean'));
gulp.task('compile', gulp.series('patternlab:compile'));
